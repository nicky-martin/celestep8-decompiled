...
with open("celeste-decompression-steps.txt", "w") as out:
    ...
    step_count = 0
    max_steps = 100  # Show first 100 steps
    
    while len(code) < decompressed_length and step_count < max_steps and code_pos < len(decimal_values):
    ...
    out.write("\n" + "=" * 80 + "\n")
    out.write(f"FINAL OUTPUT (first {step_count} steps):\n")
    out.write("=" * 80 + "\n\n")
    
    final_code = ''.join(code)
    out.write(final_code[:2000])  # Show first 2000 characters
    
    if len(code) < decompressed_length:
        out.write(f"\n\n... [truncated for readability] ...\n")
        out.write(f"\nShowing {len(code)} of {decompressed_length} total bytes\n")

print("✓ Decompression trace saved to celeste-decompression-steps.txt")
print(f"✓ Processed {len(decimal_values)} bytes from code section")
print("✓ Showing first 100 decompression steps")