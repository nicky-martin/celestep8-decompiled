...
def unsteganize_png(width, height, rows, info):
    ...
    # Open debug file for bit extraction output
    debug_file = open("celeste-bits.txt", "w")
    debug_file.write("=== CELESTE BIT-LEVEL EXTRACTION ===\n")
    debug_file.write(f"Image size: {width}x{height} pixels = {width * height} bytes of hidden data\n\n")
    ...
    for row, row_data in enumerate(rows):
        for col in range(width):
            ...
            # Write bit extraction details for every pixel
            debug_file.write(f"Pixel ({row},{col}) [byte position {row * width + col}]:\n")
            debug_file.write(f"  Original RGBA values: R={row_data[col * planes + 0]:3d} G={row_data[col * planes + 1]:3d} B={row_data[col * planes + 2]:3d} A={row_data[col * planes + 3]:3d}\n")
            debug_file.write(f"  Extracted 2 bits:     R={R:08b} G={G:08b} B={B:08b} A={A:08b}\n")
            debug_file.write(f"  Reassembled byte:     {pico_byte:08b} (decimal: {pico_byte:3d}) = '{chr(pico_byte) if 32 <= pico_byte < 127 else '?'}'\n")
            debug_file.write("\n")
    
    debug_file.write("=== EXTRACTION COMPLETE ===\n")
    debug_file.close()
    print("Bit extraction saved to celeste-bits.txt")
